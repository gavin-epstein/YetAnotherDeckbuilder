...
float value(float x, float y, float rad, PGraphics img){
  ...
  float s = 20;
  if((angle >PI-t && angle < 2*PI-t )|| (angle-2*PI>PI-t && angle-2*PI < 2*PI-t )){
    if (x> .1*img.width-s && x  < .9*img.width+s && y> .1*img.height-s && y  < .9*img.height+s
      &&(x< .1*img.width+s || x  > .9*img.width-s || y< .1*img.height+s || y  > .9*img.height-s)
    ){
    val -= 1.5;
    }
